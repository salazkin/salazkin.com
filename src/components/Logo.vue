<template>
  <div :style="containerStyle">
    <svg :style="svgStyle" :viewBox="`0 0 ${logoBounds.w} ${logoBounds.h}`">
      <path :fill="logoColor" :d="logoPath" />
    </svg>
    <div class="fill-rect" :style="fillStyle" @click="navigateToUrl"></div>
...
<script lang="ts">
import { getColorStr } from "utils/mathUtils";
...
export default {
  ...
  setup() {
    ...
    const svgStyle = computed<CSSProperties>(() => ({
      position: "absolute",
      width: `${logoBounds.w}px`,
      height: `${logoBounds.h}px`
    }));
    ...
    const tooltipStyle = computed<CSSProperties>(() => ({
      position: "absolute",
      color: getColorStr(tooltipColor),
      fontFamily: "sans-serif",
      userSelect: "none",
      pointerEvents: "none",
      top: `${logoBounds.h + tooltipOffsetY}px`
    }));

    const navigateToUrl = () => {
      window.location.href = "https://github.com/salazkin";
    };
